...
export interface SendMessageOptions {
  ...
  onPendingChange?: (path: string, before: string, after: string) => void;
  onCheckpoint?: () => void;
  onTerminalLine?: (
    content: string,
...
export function useChat(): UseChatReturn {
  ...
  const [inputStatus, setInputStatus] = useState<
    'ready' | 'submitting' | 'streaming' | 'error'
  >('ready');
  ...
  const sendMessage = useCallback(
        ...
        const response = await withRetry(
          ...
          },
          {
            ...
            onRetry: (error, attempt) => {
              ...
              onTerminalLine?.(
                `⚠ Request failed: ${error.message}. Retrying (${attempt}/3)...`,
                'error',
              );
            },
          },
              ...
              switch (eventType) {
                ...
                case 'thinking-chunk': {
                  ...
                  setMessages((prev) => {
                    ...
                    const existingThinking =
                      updated[assistantIndex].thinking || [];
                ...
                case 'code-chunk':
                  ...
                  if (hasCompleteFiles || hasInlineFileMarkers) {
                    ...
                    if (parsed.type === 'project' && parsed.files.length > 0) {
                      ...
                      // Update workspace in real-time during streaming (both Plan and Build modes)
                ...
                case 'done': {
                  ...
                  if (hasCode) {
                    ...
                    // Update workspace when code is generated (both Plan and Build modes)
                    if (onWorkspaceUpdate) {
                  ...
                  if (mode === 'plan' && streamedExplanation && onPlanParsed) {
                    const parsedPlan = parsePlanResponse(
                      streamedExplanation,
                      currentPlan as unknown as
                        | ExecutionPlan
                        | null
                        | undefined,
                    );
                  ...
                  if (hasCode) {
                    const qualityReport = checkCodeQuality(
                      data.code,
                      detectedLanguage,
                    );
                ...
                case 'tool-result': {
                  ...
                  setMessages((prev) => {
                    ...
                    const currentResults =
                      updated[assistantIndex].toolResults || '';
                ...
                case 'file-diff': {
                  ...
                  // Trigger pending change callback for review flow
    ...
    [chatId],
  ); // Only depend on chatId, not messages.length
